...
export const warezcdnScraper = makeSourcerer({
  ...
  scrapeMovie: async (ctx) => {
    ...
    const serversPage = await ctx.proxiedFetcher<string>(
      `/filme/${ctx.media.imdbId}`,
      {
    ...
    embedsHost.forEach(async (element) => {
      ...
      if (embedHost === "mixdrop") {
        const realEmbedUrl = await getExternalPlayerUrl(
          ctx,
          "mixdrop",
          embedUrl,
        );
  ...
  scrapeShow: async (ctx) => {
    ...
    const episodeId = serversPage.match(
      /\$\('\[data-load-episode-content="(\d+)"\]'\)/,
    )?.[1];
